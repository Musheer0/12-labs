...
const VoiceCard = ({ voice }: Props) => {
  ...
  return (
    <div className="flex items-center gap-1 p-3 h-fit justify-between overflow-hidden relative w-full max-w-[300px] rounded-xl gap-3 border pr-3 lg:pr-6">
      ...
      <Link
        href={`/text-to-speech?voiceId=${voice._id}`}
        className="info flex-1 mr-auto"
      >
        <p className="text-md font-semibold capitalize line-clamp-1">
          {voice.name}
        </p>
        <p
          className={cn(
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={async () => {
                    const r = await renameMutation({
                      id: voice._id as Id<"voice">,
                      name: newName,
                    });
                    if (r.success) {
